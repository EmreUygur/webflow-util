...
export class Sa5Core {
  ...
  async initScriptInjectionsAsync() {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      loadSrcScripts.forEach((script) => {
        const loadSrcUrl = script.getAttribute(
          Sa5Attribute.ATTR_CORE_SCRIPT_INJECT
        );
  ...
  initDebugMode() {
    ...
    if (hasDebug) {
      ...
      wfuDebug.persistentDebug = this.stringToBoolean(
        params.get(debugParamKey)
      );
    }
  }
  ...
  static startup(module: any | null = null): Sa5Core {
    ...
    if (sa5instance?.constructor?.name == "Sa5Core") {
      ...
      // Absorb handlers
      if (Array.isArray(sa5instance)) core.handlers = sa5instance; //window["sa5"];
