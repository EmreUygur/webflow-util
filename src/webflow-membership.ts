...
type GetConfigCallback = (Sa5MembershipConfig) => Sa5UserAccountsConfig;
...
export class Sa5UserAccounts {
  ...
  private isUserInfoChangedCallback(
    func: Function
  ): func is UserInfoChangedCallback {
  ...
  init() {
    ...
    let configHandler: GetConfigCallback = core.getHandler(
      "getMembershipConfig"
    ) as GetConfigCallback;

    if (!configHandler) return; // do nothing
    ...
    let forms = document.querySelectorAll(
      "form[data-wf-user-form-type='login']"
    );
    forms.forEach((form) => {
    ...
    forms = document.querySelectorAll(
      "form[data-wf-user-form-type='userAccount']"
    );
    forms.forEach((form) => {
  ...
  async loadUserInfoAsync_accountInfo() {
    ...
    userInfoPixel.addEventListener("load", async () => {
      ...
      if (userInfoPixel.contentDocument) {
        userAccountEmailInput = userInfoPixel.contentDocument.querySelector(
          "input#wf-user-account-email"
        );
      } else if (userInfoPixel.contentWindow) {
        userAccountEmailInput =
          userInfoPixel.contentWindow.document.querySelector(
            "input#wf-user-account-email"
          );
      }
      ...
      userAccountEmailInput.addEventListener("change", async () => {
        ...
        this.debug.debug(
          `waiting ${this.config.advanced.accountInfoLoadDelay}ms`
        );

        setTimeout(async () => {
          this.debug.debug(
            `%c USER-ACCOUNT LOADED`,
            "color: #ff0000; background-color: yellow;"
          );
          ...
          // Get pixel
          let doc =
            userInfoPixel.contentDocument ||
            userInfoPixel.contentWindow?.document;

          if (doc) {
            let userNameElement = doc.querySelector(
            ...
            let acceptCommunicationsCheckbox = doc.querySelector(
            ...
            userDataFields.forEach((element) => {
              ...
              if (id.startsWith("wf-user-account-")) return;
  ...
  async loadUserInfoAsync_accessGroups() {
    ...
    if (this.config.accessGroups) {
      let accessGroupHandler: Sa5UserAccessGroups = new Sa5UserAccessGroups(
        this
      );
  ...
  saveUserInfoCache(newUserData) {
    ...
    if (!userData) userData = new Sa5User();
  ...
  expandLoginButton(elem: HTMLElement) {
    ...
    const wfLoginButton: HTMLElement = elem.querySelector(
      "[data-wf-user-logout]"
    );
