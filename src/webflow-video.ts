...
export class WebflowVideo {
  ...
  processAllDataPosterUrls(): void {
    ...
    const elements = document.querySelectorAll(
      `div[${Sa5Attribute.ATTR_VIDEO_DATA_POSTER_URL}]`
    );
  ...
  processAllYouTubeNorel(): void {
    ...
    for (let playerWrap of document.querySelectorAll<HTMLElement>(
      `[${Sa5Attribute.ATTR_VIDEO_YOUTUBE_NOREL}]`
    )) {
      ...
      if (!playerWrap.classList.contains("w-embed-youtubevideo")) {
        console.warn(
          "SA5",
          "The SA5 YouTube Norel attribute can only be applied to Webflow's YouTube elements."
        );
      ...
      if (!playerFrame) {
        console.warn(
          "SA5",
          "YouTube player element malformed, missing iframe."
        );
        continue; // skip to next
      }
