...
export class Sa5Datastore {
  ...
  private isDatastoreLoadedCallback(
    func: Function
  ): func is DatastoreLoadedCallback {
  ...
  init() {
    ...
    const datastoreLoaded = core.getHandler(
      Sa5GlobalEvent.EVENT_DATASTORE_LOADED
    );
  ...
  loadDataItem_v2(elem: HTMLElement): void {
    ...
    if (!this.store[dsn]) this.store[dsn] = new Database();
