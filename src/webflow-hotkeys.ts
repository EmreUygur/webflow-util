...
export class Sa5Hotkeys {
  ...
  registerModifier(
    ...
    document.addEventListener("keydown", function (event) {
      if (event.key === "Control" && !isCtrlDown) {
        ...
        timer = setTimeout(function () {
          ...
          if (callbackTrigger) callbackTrigger(modifier, "keydown", event);
    ...
    document.addEventListener("keyup", function (event) {
      if (event.key === "Control") {
        ...
        if (callbackRelease) callbackRelease(modifier, "keyup", event);
  ...
  register(combination: string, callback: (arg: any) => void) {
