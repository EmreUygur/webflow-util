...
export class WfuDataBinder {
  ...
  bindAll() {
    ...
    let dataBind = document.querySelectorAll(`[${Sa5Attribute.ATTR_DATABIND}]`); // wfu-bind
    ...
    let dataBindContent = document.querySelectorAll(
      `[${Sa5Attribute.ATTR_DATABIND_CONTENT}]`
    ); // wfu-bind-content
  ...
  bind(elem: HTMLElement) {
    ...
    switch (identifyElement(elem)) {
      ...
      case "HTMLSelectElement":
        selectOptionByValue(elem as HTMLSelectElement, val);
    ...
    switch (dsd.type) {
      ...
      case DataSourceTypeEnum.DB:
        let dsnContext = this.findContextSetting(
          elem,
          Sa5Attribute.ATTR_DATABIND_CONTEXT_DSN
        ); // "wfu-bind-dsn"
        let itemContext = this.findContextSetting(
          elem,
          Sa5Attribute.ATTR_DATABIND_CONTEXT_ITEM_ID
        ); // "wfu-bind-item-id"
  ...
  private getData_user(dsd: Sa5DataSourceDescriptor): string {
    ...
    // TODO: warn?
    if (!this.config.user || !this.config.user.email) return null;
