...
export class Sa5Modal {
  ...
  isSuppressed() {
    ...
    const suppressed = getCookie(this.getModalKeyName());
  ...
  suppress(val, duration: number) {
    setCookie(this.getModalKeyName(), val, {
  ...
  unSuppress() {
    // We'll use session storage
    // suppression is temporary, to the current tab-session
    removeCookie(this.getModalKeyName());
  }
